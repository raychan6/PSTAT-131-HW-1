...
Bias is the error introduced by estimating a problem with a simpler model. Variance is the amount by which $\hat{f}$ would change if we obtained it with a different data set. As we use more flexible methods, the variance will increase, and the bias will decrease. However, at some point, increasing flexibility has little impact on bias but significantly increases the variance. We need to find a method/model for which the variance and squared bias are both low.  
